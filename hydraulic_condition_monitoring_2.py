...
class DFNN_Model(nn.Module):
    def __init__(self,input_features=17,hidden1=10,hidden2=10,hidden3=10,hidden4=10,out_features=5):
        super().__init__()
    ...
    def forward(self,x):
        x=F.relu(self.f_connected1(x),xxxx)
        x=F.relu(self.f_connected2(x),xxxx)
        x=F.relu(self.f_connected3(x),xxxx)
        x=F.relu(self.f_connected4(x),xxxx)
        x=self.out(x)
        return x
